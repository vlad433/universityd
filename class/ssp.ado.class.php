  ...
  for ($i=0, $ien=count($data); $i<$ien; $i++) {
   ...
   for ($j=0, $jen=count($columns) ; $j<$jen ; $j++) {
    ...
    if (isset( $column['formatter'])) {
     $row[ $column['dt'] ] = ($isJoin) ? $column['formatter']($data[$i][$column['field']], $data[$i]) 
                                       : $column['formatter']($data[$i][$column['db']], $data[$i]);
    } else {
     $row[$column['dt']] = ($isJoin) ? $data[$i][$columns[$j]['field']]
                                     : $data[$i][$columns[$j]['db']];
    }
   }
   $out[] = $row;
...
///     $orderBy[] = ($isJoin) ? $column['db'].' '.$dir : '`'.$column['db'].'` '.$dir;
    if (isset( $column['sort'])) {$orderBy[] = $column['sort'].' '.$dir;}
    else {$orderBy[] = $column['db'].' '.$dir;}
    }
   }
...
///     $globalSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding 
...
//    $binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    $binding = SSP::bind( $bindings, '%'.$str.'%');
///    $columnSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding 
///                                : "`".$column['db']."` LIKE ".$binding;
    $columnSearch[] = $column['db']." LIKE ".$binding;
///    $columnSearch[] = $column['db']." LIKE CONVERT(".$binding." USING utf8)";
   }
  }
 ...
 static function simple ($conn, $request, $table, $primaryKey, $columns, 
